...
WP_CID=$(docker create --link $SQL_CID:mysql -p 80 -v /run/lock/apache2/ -v /run/apache2/ -v /tmp/ lindison/wordpress)
docker start $WP_CID

WP2_CID=$(docker create --link $SQL_CID:mysql -p 80 -v /run/lock/apache2/ -v /run/apache2/ -v /tmp/ lindison/wordpress)
docker start $WP2_CID
